...
class Transformer(nn.Module):
    ...
    def forward(self, src, target, src_mask, tgt_mask, src_padding_mask, tgt_padding_mask, memory_key_padding_mask, training=False):        
        ...
        #mean_decoder_out = torch.mean(real_output*~tgt_padding_mask.unsqueeze(-1), dim=1)
...
def train_model(config, train_dataloader, valid_dataloader, model_checkpoint_path):
    ...
    #foldername = config['data_size'] + "_wEOS_95_lr_" + str(config['learning_rate']) + "_batchsize_" + str(config['batch_size']) + "_dropout_" + str(config['dropout'])
    foldername = "TEST"
